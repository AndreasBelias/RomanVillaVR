...
def initGroups(initFlag=vizconnect.INIT_INDEPENDENT, initList=None):
	...
	if vizconnect.isPendingInit('group', _name, initFlag, initList):
		...
		if initFlag&vizconnect.INIT_PARENTS:
			vizconnect.getGroup(_name).setParent(vizconnect.getAvatar('main_avatar').getAttachmentPoint('r_hand'))
		...
		if initFlag&vizconnect.INIT_MAPPINGS:
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				def update(transport):
					if rawInput['l_hand_input'].getTrackpad()[1] > 0.01:# make=HTC, model=Vive Controller, name=l_hand_input, signal=Trackpad Top
						transport.moveForward(mag=abs(rawInput['l_hand_input'].getTrackpad()[1]))
					if rawInput['l_hand_input'].getTrackpad()[1] < -0.01:# make=HTC, model=Vive Controller, name=l_hand_input, signal=Trackpad Bottom
						transport.moveBackward(mag=abs(rawInput['l_hand_input'].getTrackpad()[1]))
					if rawInput['l_hand_input'].getTrackpad()[0] < -0.01:# make=HTC, model=Vive Controller, name=l_hand_input, signal=Trackpad Left
						transport.moveLeft(mag=abs(rawInput['l_hand_input'].getTrackpad()[0]))
					if rawInput['l_hand_input'].getTrackpad()[0] > 0.01:# make=HTC, model=Vive Controller, name=l_hand_input, signal=Trackpad Right
						transport.moveRight(mag=abs(rawInput['l_hand_input'].getTrackpad()[0]))
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				def update(transport):
					if rawInput['r_hand_input'].isButtonDown(3):# make=HTC, model=Vive Controller, name=r_hand_input, signal=Button Trackpad
						transport.start()
					if not rawInput['r_hand_input'].isButtonDown(3):# make=HTC, model=Vive Controller, name=r_hand_input, signal=Button Trackpad
						transport.stop()
				rawTransport[_name].setUpdateFunction(update)
